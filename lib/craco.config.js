...
const enableCKEWebpackConfigPlugin = (webpackConfig, {
  env,
  paths
}) => {
  ...
  loader.exclude = [cssModuleRegex, /ckeditor5-[^/\\]+[/\\]theme[/\\].+\.css$/]; // Modify cssModuleRegex

  loader = oneOf.find(item => {
    if (item.test !== undefined) return item.test.toString() === cssModuleRegex.toString();
  });
  loader.exclude = [/ckeditor5-[^/\\]+[/\\]theme[/\\].+\.css$/]; // Modify file-loader

  loader = oneOf.find(item => {
    if (item.loader !== undefined) return item.loader.toString() === require.resolve("file-loader").toString();
  });
  loader.exclude = [/\.(js|mjs|jsx|ts|tsx)$/, /\.html$/, /\.json$/, /ckeditor5-[^/\\]+[/\\]theme[/\\]icons[/\\][^/\\]+\.svg$/, /ckeditor5-[^/\\]+[/\\]theme[/\\].+\.css$/]; // Always return a config object.
